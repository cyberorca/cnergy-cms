                ...
                <div class="col-md-12">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="basicInput"
                            name="title" placeholder="Enter menu name" @if ($method==='edit' )
                            value="{{ $fe_menu->title }}" @endif />
                        @error('title')
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-select" multiple aria-label="multiple select example" name="position[]">
                            <option value="header" @if ($method==='edit' ) @if (in_array('header',
                                json_decode($fe_menu->position, true)))
                                selected @endif
                                @endif>Header</option>
                            <option @if ($method==='edit' ) @if (in_array('footer', json_decode($fe_menu->position,
                                true)))
                                selected @endif
                                @endif
