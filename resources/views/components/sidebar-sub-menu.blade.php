<li
    ...
    @if (str_contains(request()->path() . "/", $item['slug']) && count($item['children']) == 0) data-status-sidebar="true" @endif>

    <a @if (count($item['children'])) class="sidebar-link"
        href="{{ url($item['slug']) }}"
    @else
        href="{{ url($item['slug']) }}"
        @if (str_contains(request()->path() . "/", $item['slug'])) style="background-color: #f0f1f5 !important; border-radius: 7px" @endif
    @endif
        >
        {{ $item['menu_name'] }}
    </a>
